 ...
 */
public class InventoryCafeReport extends Task<Workbook> {
    ...
    private HashMap<Integer, ArrayList<InventoryCafe>> current;
    private HashMap<Integer, ArrayList<InventoryCafe>> previous;
    ...
    public void setInv(InventoryCafe inv) {

        ids = new HashSet<>();
        current = new HashMap<>();
        previous = new HashMap<>();
        ingredientsList = GlobalPandaApp.site.getIngredients();
        for ( InventoryCafe cur : GlobalPandaApp.site.getInventoryCafeList(inv.getCheckId()) ) {
            ...
            if ( !previous.containsKey(cur.getBasicIng()))
                previous.put(cur.getBasicIng(), GlobalPandaApp.site.getLastCafeInventory(cur.getBasicIng(), cur.getCheckId(), cur.getKitchen()));
        }
    ...
    private void fillTotalSheet () {
        try {
            ...
            for (Integer i : current.keySet()) {
                ArrayList<InventoryCafe> cur = current.get(i);
                ArrayList<InventoryCafe> prev = previous.get(i);
                ...
                for (InventoryCafe c : cur) {
                    Calendar fromCal = Calendar.getInstance();
                    Calendar toCal = Calendar.getInstance();
                    fromCal.setTime(c.getPrevious());
                    fromCal.setTime(c.getBegin());
                    int totalTushka = 0;

                    ids.add(c.getProdIngId());

                    Optional<InventoryCafe> p = prev.stream().filter(ing -> ing.getProdIngId().equals(c.getProdIngId())).findFirst();
                    ...
                    row.createCell(19, Cell.CELL_TYPE_NUMERIC).setCellValue(GlobalPandaApp.site.getNettoConsumptionCafeBetweenDates(c.getProdIngId(), fromCal, toCal));
                    ...
                    if (c.getProdIngId() < 1700) {
                        ...
                        int sumCons = GlobalPandaApp.site.getNettoConsumptionCafeBetweenDates(c.getProdIngId(), fromCal, toCal);
                        ...
                        float nfCoef = GlobalPandaApp.site.getCoefCafeNf(c.getBasicIng(), c.getProdIngId());
                ...
                if (totalConsumption > 0) {
                    ...
                    float cost = GlobalPandaApp.site.getIngCafeCost(cur).floatValue();
    ...
    private void fillConsumptionSheet () {
        ...
        header.createCell(1, Cell.CELL_TYPE_STRING).setCellValue("Страва");
        header.getCell(1).setCellStyle(headerStyle);

        header.createCell(2, Cell.CELL_TYPE_STRING).setCellValue("К-ть");
        header.getCell(2).setCellStyle(headerStyle);

        int col = 3;
        ArrayList<INGREDIENTS> ingredientses = GlobalPandaApp.site.getCafeIngredients();
        for (Integer i : current.keySet()) {
            ArrayList<InventoryCafe> cur = current.get(i);
            for (InventoryCafe inv : cur) {
        ...
        for (Integer i : current.keySet()) {
            ...
            for (InventoryCafe inv : cur) {
                ...
                ArrayList<ConsumptionCafe> cons =
                        GlobalPandaApp.site.getIngredientConsumptionCafeList(from, inv.getBegin(), inv.getProdIngId(), inv.getKitchen());
                for (ConsumptionCafe con : cons) {
                    if (!dishMap.containsKey(con.getId())) {
                        ConsReport rep = new ConsReport();
                        rep.setDate(con.getDate());
                        rep.setDish(con.getDishName());
                        rep.setCount(con.getCount());
                        dishMap.put(con.getId(), rep);
                    }

                    if (!dishMap.get(con.getId()).getCountIng().containsKey(con.getIngredientId()))
                        dishMap.get(con.getId()).getCountIng().put(con.getIngredientId(), 0);

                    dishMap.get(con.getId()).addCountIng(con.getIngredientId(), (int) con.getWeight());
                }
            }
        }
        ...
        for (ConsReport cRep : list) {
            ...
            row.createCell(1, Cell.CELL_TYPE_STRING).setCellValue(cRep.getDish());

            row.createCell(2, Cell.CELL_TYPE_NUMERIC).setCellValue(cRep.getCount());
            row.getCell(2).setCellStyle(intNumStyle);

            int colNum = 3;
            for (int i : current.keySet()) {
                for (InventoryCafe inv : current.get(i)) {
                    if (inv.getProdIngId() < 1900) {
    ...
    private void fillRozrobkaSheet () {
        ...
        for (Integer i : current.keySet()) {
            ArrayList<InventoryCafe> list = current.get(i);
